...
and scan whatever the array can lock onto.
...
SCANNING
--------
Objects answer on different bands. A red giant shows up on THERMAL, a quasar
screams on RADIO, most stars are simple VISUAL targets. Scanning on the wrong
band gives you a weak, slow, noisy return -- the ring will barely creep.
...
MONITOR_SLOTS = 3          # max simultaneous monitor windows
MONITOR_HIST_LEN = 64      # samples kept per monitor graph
MONITOR_SAMPLE_DT = 0.15   # seconds between graph samples (keeps it cheap)
DIAL_SWEEP_DEG = 132.0     # +/- degrees either side of straight-up for the Z dial
...
# screen size is decided at runtime to fit the desktop
SW, SH = 1280, 760
...
OMEGA_L = 0.690
C_KMS = 299792.458
HUBBLE_TIME_GYR = 977.79 / H0          # 1/H0 expressed in Gyr
HUBBLE_DIST_MPC = C_KMS / H0           # c/H0 in Mpc
MPC_TO_MLY = 3.26156                   # Mpc -> million light years

_COS_Z = []
_COS_DC = []      # comoving distance, Mpc
_COS_TL = []      # lookback time, Gyr


def _E(z):
    """Dimensionless Hubble parameter H(z)/H0 for a flat LCDM universe."""
    return math.sqrt(OMEGA_M * (1.0 + z) ** 3 + OMEGA_L)


def _build_cosmology_tables():
    """Numerically integrate comoving distance and lookback time once."""
    global _COS_Z, _COS_DC, _COS_TL
    zmax, steps = 1200.0, 6000
    # log-spaced sampling so low z (where everything interesting lives) is dense
    zs = [0.0]
...
def light_travel_mly(z):
    """Lookback time expressed as a light-travel distance, in million ly."""
    return lookback_gyr(z) * 1000.0


def recession_kms(z):
    """Relativistic recession velocity from redshift (special-relativistic form)."""
    a = (1.0 + z) ** 2
...
def z_from_distance_mly(d_mly):
    """Invert the comoving distance relation for a distance in million ly."""
...
# emission lines used by the spectrometer readout (rest wavelengths, nm)
SPECTRAL_LINES = [
...
# ==========================================================================
# CATALOGS
# Published RA / Dec figures, rounded. Distances and redshifts are the
# commonly quoted values. Close enough that the sky is recognisable.
# ==========================================================================
...
# name, RA hours, Dec deg, distance ly, apparent mag, spectral class
STARS_RAW = [
...
# Deep sky: name, RA h, Dec deg, redshift z, kind, band, blurb
# kind: galaxy / cluster / quasar / nebula / globular / open / remnant / exotic / structure
DEEP_SKY_RAW = [
    ...
    # local nebulae / clusters / remnants -----------------------------------
    ...
    ("Cassiopeia A", 23.3911, 58.8150, 0.0, "remnant", "RADIO",
     "The brightest radio source outside the solar system. The light of its explosion should have been visible around 1680. Nobody recorded it."),
...
# ANOMALIES
# ==========================================================================
# band      : sensor mode that will actually lock onto it
# trait     : ""        normal
#             "flicker" drops in and out of the feed
#             "shy"     drifts away from the reticle, easy to lose
#             "edge"    only renders in peripheral vision -- look straight at
#                       it and it is not there
# classify  : line logged by the first scan
# resolve   : line logged by the follow-up scan, hours later
...
BOOT_LINES = [
    "VYSTLZ GROUND ARRAY  --  DEEP SKY RECEIVER  --  BIOS v2.0",
    "MEM CHECK ................ OK",
    "DISH SERVO ............... OK",
...
# (stat key, threshold, message) -- small score bumps to reward long sessions
MILESTONES = [
...
# SECRET DEBUG MODE
# ==========================================================================
# Type the game's own name, one key at a time, with no more than a couple of
# seconds between keys. Nothing in normal play ever presses V or Y, so this
# cannot fire by accident. Backtick toggles the panel once unlocked.

DEBUG_SEQ = [pygame.K_v, pygame.K_y, pygame.K_s, pygame.K_t, pygame.K_l, pygame.K_z]
DEBUG_KEY_WINDOW = 2.5     # seconds allowed between consecutive keys

# (label, action key) -- rendered as buttons in the debug panel
DEBUG_ACTIONS = [
...
def gal_lat(ra_deg, dec_deg):
    """Galactic latitude, for placing the Milky Way band realistically."""
...
class SkyObject:
    ...
    def __init__(self, oid, name, ra, dec, z, kind, band, blurb="", mag=6.0,
        ...
        self.trait = trait
        self.stage = 0            # 0 unscanned, 1 classified, 2 resolved
        self.stage_time = 0.0     # game-hours at which stage 1 happened
        ...
        self.cause = None
        self.proc = proc
        self.spawn = False        # spawned at runtime (clustering / transient)
        self.cluster_id = -1
        self.note = ""
        self.born = 0.0
...
class World:
    """Everything in the sky, plus a coarse RA/Dec grid for fast culling."""
    ...
    def __init__(self, seed):
        ...
        self.star_by_name = {}
        self.next_oid = 1
        self.attention = {}     # grid cell -> scans made there (drives clustering)
        self._build()
    ...
    def _build(self):
        ...
        # a handful of stars get a genuine periodic dimming mystery
        ...
        # --- procedural field stars (Milky Way band) -----------------------
        made = 0
        while made < 3400:
            ra = rng.uniform(0, 360)
            dec = math.degrees(math.asin(rng.uniform(-1, 1)))
            b = abs(gal_lat(ra, dec))
            # concentrate toward the galactic plane
        ...
        for ci, (cra, cdec, cz, spread) in enumerate(clusters):
            ...
            for _ in range(n):
                ...
                o.cluster_id = ci
                self._add(o)
        # sparse field galaxies everywhere else
        for _ in range(1500):
            ra = rng.uniform(0, 360)
            dec = math.degrees(math.asin(rng.uniform(-1, 1)))
    ...
    # ---- runtime additions ------------------------------------------------
    def spawn_anomaly(self, ra, dec, z, game_hours, type_data=None, note=""):
...
# ==========================================================================
# AUDIO  (all synthesized at runtime; silently degrades to nothing)
# ==========================================================================
...
class Audio:
    ...
    def _noise_loop(self, dur, vol):
        ...
        # crude low-pass so it hisses rather than screams
        k = 12
    ...
    def _voice(self, idx):
        """Something that is almost, but not quite, a word."""
        dur = 1.1 + idx * 0.13
        n = int(self.rate * dur)
        t = np.linspace(0, dur, n, False)
        f0 = 96 + idx * 11
        ...
        # two drifting formants -> vowel-ish
...
def bar_str(frac, width=8):
    """Tiny text gauge, e.g. [###.....], for CRT-style instrument readouts."""
...
def glow_sprite(radius, color):
    """Cached additive glow blob. Radius is bucketed to keep the cache small."""
    r = max(1, int(radius))
...
def make_noise_tiles(w, h, count=5):
    """Pre-render a few noise frames; cycling them is far cheaper than per-frame noise."""
    tiles = []
...
def make_static_frames(w, h, count=4):
    """Heavy interference static, used when the carrier drops."""
    frames = []
    for _ in range(count):
        s = pygame.Surface((w, h), pygame.SRCALPHA)
        if HAVE_NUMPY:
...
class Layout:
    """All geometry derives from the window size so it fits any laptop."""
    ...
    def recompute(self, w, h):
        self.w, self.h = w, h
        self.scale = clamp(h / 760.0, 0.72, 1.35)
        self.top_h = int(40 * self.scale)
        self.bottom_h = int(86 * self.scale)
        self.panel_w = int(clamp(w * 0.26, 250, 380))
...
class Game:
    def __init__(self):
        ...
        w = int(clamp(dw * 0.88, 960, 1600))
        h = int(clamp(dh * 0.86, 620, 980))
        global SW, SH
        SW, SH = w, h
        ...
        # CRT glitch-shift offset -- referenced by render_crt() every frame,
        # including while the main menu is up (before new_session() has run),
        # so it must exist from the moment the window opens.
        self.shift = 0.0

        self.state = "MENU"
        self.menu_page = "MAIN"       # MAIN / LOAD / SAVE / SETTINGS / ABOUT
        self.menu_sel = 0
        self.menu_msg = ""
        self.menu_msg_t = 0.0
        self.menu_t = 0.0
        ...
        self.debug_keybuf = []       # [(key, timestamp), ...]
        self.debug_reveal = False    # when True, sky shows everything
        self.debug_rects = []        # populated by render_debug()
    ...
    def toggle_fullscreen(self):
        ...
        if self.fullscreen:
            ...
            info = pygame.display.Info()
            w = int(clamp(info.current_w * 0.88, 960, 1600))
            h = int(clamp(info.current_h * 0.86, 620, 980))
            self.screen = pygame.display.set_mode((w, h), pygame.RESIZABLE | pygame.DOUBLEBUF)
        global SW, SH
        SW, SH = w, h
        self.L.recompute(w, h)
    ...
    def new_session(self, seed=None):
        ...
        # --- randomised starting orientation --------------------------------
        # Every survey opens pointed somewhere different, as if the array had
        # been left tracking wherever it last was -- pure replayability, no
        # effect on the catalogue itself.
        self.cam_ra = random.uniform(0.0, 360.0)
        self.cam_dec = random.uniform(-58.0, 58.0)
        self.zoom = random.uniform(7.5, 11.5)      # pixels per degree
        self.z_dial = 0.0
        self.dial_p = 0.0                         # dial position 0..1 (log mapped)
        self.mode = "VISUAL"
        self.game_hours = 6.0
        self.score = 0
        self.log = []
        self.filter_i = 0
        self.tab = 0                              # 0 LOG 1 DATA 2 CALIB 3 HELP
        self.scan_p = 0.0
        self.scan_need = 1.0
        self.target = None
        self.hold_pos = None
        ...
        # --- monitor scan (crosshair mode toggled with R) -------------------
        self.monitor_mode = False
        self.monitors = []            # list of {oid, hist, flash, sample_t, rect, close_rect}
        self.mon_target = None
        self.mon_scan_p = 0.0
        self.mon_hold_pos = None
        self.mon_need = 0.6

        # --- redshift dial geometry (computed each frame by render_bottom) --
        self.dial_center = (0, 0)
        self.dial_R = 1

        # --- session milestones (replayability -- small score bumps) -------
        self.milestones_done = set()

        self.dupe = None                          # (obj, t_left, offset)
        ...
        # per-frame UI hit rects (populated by render, read by input)
        ...
        self.state = "BOOT"
        self.boot_t = 0.0
        self.log_add("SESSION START -- observation only. no survival subsystems present.", PHOS_DIM, CAT_SYS)
        self.log_add("Array online. Redshift tuner at z = 0.000. Sensor: VISUAL.", PHOS_DIM, CAT_SYS)
    ...
    def save_game(self, slot):
        try:
            ...
            spawned = [[o.oid, o.name, round(o.ra, 5), round(o.dec, 5), o.z, o.band,
                        o.trait, o.blurb, o.note, round(o.born, 3)]
    ...
    # dial mapping: p in 0..1 -> z in 0..1089 (logarithmic, so the local sky
    # occupies a usable amount of travel)
    def p_to_z(self, p):
    ...
    def sun_pos(self):
        """Approximate solar RA/Dec from the in-game date -- drives glare."""
    ...
    def band_strength(self, obj):
        """How well the current dial setting picks this object up (0..1)."""
        if abs(obj.z) < 1e-6:
            # local sky: fades out as you tune the dial away from z = 0
    ...
    def on_key(self, e):
        ...
        # --- secret debug unlock (V-Y-S-T-L-Z), works from any state ---
        now = time.time()
        self.debug_keybuf = [x for x in self.debug_keybuf
                             if now - x[1] < DEBUG_KEY_WINDOW]
        self.debug_keybuf.append((k, now))
        if len(self.debug_keybuf) >= len(DEBUG_SEQ) and \
                [x[0] for x in self.debug_keybuf[-len(DEBUG_SEQ):]] == DEBUG_SEQ:
            self.debug_keybuf = []
            self.debug_rects = []
        ...
        if self.debug_unlocked and k == pygame.K_BACKQUOTE:
            self.debug_open = not self.debug_open
            self.debug_rects = []
            return
        ...
        if k in (pygame.K_ESCAPE, pygame.K_m):
            ...
            if self.monitor_mode:
                self.toast = "MONITOR SCAN -- hold LMB on a contact to link it"
            else:
                self.toast = "MONITOR SCAN OFF"
            self.toast_t = 2.4
        elif k == pygame.K_h:
    ...
    def on_click(self, e):
        ...
        if e.button == 2:
            ...
            # debug panel first, so it sits above every other clickable thing
    ...
    def debug_action(self, key):
        ...
        if key == "interf_on":
            ...
            ra, dec = self.screen_to_world(*pygame.mouse.get_pos())
            o = w.spawn_anomaly(ra, dec, 0.0, self.game_hours, note="transient")
            o.blurb = "Contact is unstable. Signal decaying."
            self.transients.append(o)
        elif key == "reveal":
            ...
            for o in list(w.objects):
                if self.ang_sep(ra, dec, o.ra, o.dec) <= 20 and not self.obj_done(o):
                    ...
                    n += 1
            self.log_add("DEBUG: catalogued %d contacts within 20 deg" % n,
                         CYAN, CAT_SYS)
    ...
    def render_debug(self):
        ...
        text_at(self.screen, f,
                "z=%.3f  mode=%s  interf=%s  seed=%d" %
                (self.z_dial, self.mode, "Y" if self.interference else "n", self.seed),
    ...
    def update(self, dt):
        ...
        # --- WASD / arrow panning (degrees per second, scaled to zoom) -----
        ...
        if self.tab == 2 and self.calib:
            dx = dy = 0          # arrows belong to the minigame while it is open
        if dx:
    ...
    def update_events(self, dt):
        ...
        # duplicate catalogue entry -- a star briefly shows twice, then nothing
        self.dupe_cd -= dt
        ...
        # the hum, very occasionally, almost says something
        self.voice_cd -= dt
        if self.voice_cd <= 0:
            self.voice_cd = random.uniform(210, 520)
            self.audio.voice(pan=random.uniform(0.15, 0.85))

        # transient contacts: short-lived, unstable, gone if you dawdle
        self.transient_cd -= dt
        if self.transient_cd <= 0:
            self.transient_cd = random.uniform(90, 190)
            ra = (self.cam_ra + random.uniform(-14, 14)) % 360
            dec = clamp(self.cam_dec + random.uniform(-10, 10), -88, 88)
            o = self.world.spawn_anomaly(ra, dec, 0.0 if random.random() < 0.7 else self.z_dial,
                                         self.game_hours, note="transient")
            o.blurb = "Contact is unstable. Signal decaying."
            self.transients.append(o)
            self.log_add("TRANSIENT CONTACT -- unstable lock, will not hold long", AMBER, CAT_ANOM, o.oid)

        # expire un-scanned transients
        if self.transients:
            expired = [o for o in self.transients
                       if o.stage == 0 and self.game_hours - o.born > 2.6]
            for o in expired:
                self.transients.remove(o)
                if o in self.world.objects:
                    self.world.objects.remove(o)
                self.world.by_id.pop(o.oid, None)
            self.transients = [o for o in self.transients if o.stage == 0]
            if expired:
                self.world.reindex()
                self.log_add("Transient contact decayed before lock. Gone.", PHOS_DIM, CAT_SYS)

        # rare signal interference -> forces the calibration minigame
        if not self.interference:
            self.next_interf -= dt
    ...
    def start_interference(self):
        ...
        self.log_add("!! CARRIER LOST -- BROADBAND INTERFERENCE ACROSS ALL BANDS", RED_WARN, CAT_SYS)
        self.log_add("Open CALIB tab (TAB) and re-phase the three receiver channels.", AMBER, CAT_SYS)
    ...
    def calib_click(self, pos):
        for i, r in enumerate(getattr(self, "calib_rects", [])):
            if r.collidepoint(pos):
                self.calib["sel"] = i
                self.calib_drag = True
                b = self.calib["bands"][i]
                if not b["lock"]:
                    b["cur"] = clamp((pos[0] - r.x) / max(1, r.w), 0, 1)
                return
    ...
    def update_calibration(self, dt, keys):
        ...
        if getattr(self, "calib_drag", False):
            mx = pygame.mouse.get_pos()[0]
            r = getattr(self, "calib_rects", [])
            if r and c["sel"] < len(r) and not b["lock"]:
                b["cur"] = clamp((mx - r[c["sel"]].x) / max(1, r[c["sel"]].w), 0, 1)
    ...
    def find_target(self, mouse):
        ...
        best, best_d = None, 24.0 * self.L.scale
    ...
    def signal_strength(self, mouse):
        """Proximity to the nearest valid contact, for the little meter."""
        ra, dec = self.screen_to_world(*mouse)
        best = 0.0
        for o in self.world.nearby(ra, dec, 1):
            if self.obj_done(o):
    ...
    def update_scanning(self, dt, mouse, down):
        ...
        # shy contacts edge away from the reticle
        ...
        if self.hold_pos is None:
            self.hold_pos = mouse
        if math.hypot(mouse[0] - self.hold_pos[0], mouse[1] - self.hold_pos[1]) > 12 * self.L.scale:
            self.hold_pos = mouse
    ...
    def monitor_hit(self, pos):
        """True if pos is over one of the open monitor windows (UI, not sky)."""
    ...
    def find_target_monitor(self, mouse):
        """Like find_target, but liberal: anything visible answers, since
        monitoring doesn't care which band is currently tuned in."""
        ra, dec = self.screen_to_world(*mouse)
    ...
    def update_monitor_scan(self, dt, mouse, down):
        ...
        if math.hypot(mouse[0] - self.mon_hold_pos[0], mouse[1] - self.mon_hold_pos[1]) > 14 * self.L.scale:
            self.mon_hold_pos = mouse
            self.mon_scan_p = max(0.0, self.mon_scan_p - dt * 3.0)
            return
    ...
    def monitor_signal(self, o):
        """Cheap synthetic 'activity' trace, unique per object, for the graph."""
        t = self.session_t
    ...
    def open_monitor(self, o):
        ...
        if len(self.monitors) >= MONITOR_SLOTS:
            ...
            if oo:
                self.log_add("MONITOR LINK DROPPED: %s (slot needed)" % oo.name, PHOS_DIM, CAT_SYS)
    ...
    def complete_scan(self, o):
        ...
        if o.kind == "star":
            if o.variable and o.stage >= 2 and o.is_dimmed(self.game_hours):
                ...
                self.score += 120
                self.stats["resolved"] += 1
                self.log_add("DIM CYCLE SOLVED [%s]: %s" % (o.name, o.cause), AMBER, CAT_ANOM, o.oid)
                ...
                extra = ""
                if o.variable:
                    extra = "  -- periodic dimming detected, rescan during a dip"
                self.log_add("CATALOGUED: %s  %s  %s%s" %
                             (o.name, o.distance_text(), "class " + (o.note or "?"), extra),
                             PHOS, CAT_STAR, o.oid)
            ...
            if o.stage == 0:
                ...
                self.score += 90
                self.stats["anom"] += 1
                tag = (" near %s" % o.note) if o.note and o.note != "transient" else ""
                ...
                self.log_add("ANOMALY RESOLVED [%s]: %s" % (o.kind, t["resolve"] if t else "resolved."),
                             AMBER, CAT_ANOM, o.oid)
            ...
            self.log_add("CATALOGUED: %s  [%s]  z=%.4f  %s -- %s" %
                         (o.name, o.kind.upper(), o.z, o.distance_text(), desc),
    ...
    def maybe_cluster(self, o):
        """Scan a region enough and the sky starts answering back."""
        ...
        if n >= 4 and n % 3 == 1 and random.random() < 0.6:
            ...
            a = self.world.spawn_anomaly(ra, dec, o.z, self.game_hours, note=o.name)
            self.log_add("SECONDARY CONTACT resolving near %s. It was not there before." % o.name,
                         AMBER, CAT_ANOM, a.oid)
    ...
    def render_boot(self):
        ...
        for i in range(n):
            ...
            text_at(self.screen, self.fonts["sm"], line, (int(self.L.w * 0.18), y), col)
            y += int(22 * self.L.scale)
        if n >= len(BOOT_LINES) and int(self.boot_t * 2) % 2 == 0:
            text_at(self.screen, self.fonts["sm"], "_", (int(self.L.w * 0.18), y), AMBER)
        text_at(self.screen, self.fonts["xs"], "press space", (int(self.L.w * 0.18), self.L.h - 60), PHOS_DIM)
    ...
    def render_sky(self):
        ...
        # zoomed out you only get the bright sky, exactly like a real eyepiece
        ...
        for cell in cells:
            for o in cell:
                ...
                x, y = self.world_to_screen(o.ra, o.dec)
                if not (-60 <= x <= self.L.w + 60 and self.L.sky.top - 60 <= y <= self.L.sky.bottom + 60):
                    continue
                drawn += 1
                if o.kind == "star":
        ...
        # duplicate-star glitch: a second copy, slightly offset, then nothing
        if self.dupe:
            o, t, off = self.dupe
            x, y = self.world_to_screen(o.ra, o.dec)
            a = clamp(t / 1.2, 0, 1)
            if a > 0.05:
                ...
                self.screen.blit(s, (x + off[0] - s.get_width() // 2,
                                     y + off[1] - s.get_height() // 2), special_flags=pygame.BLEND_RGBA_ADD)

        self.render_sun_glare()
        self.screen.set_clip(clip)

    def render_deep_background(self):
        """Static faint field + CMB wall when the dial is tuned to the edge.
        The point field only depends on the (coarse) camera cell and window
        size, so it is cached instead of re-rolling 90 randoms every frame."""
        ...
        if self.z_dial > 200:
            # approaching the surface of last scattering
    ...
    def _cmb_surface(self):
        """CMB temperature-fluctuation wall, built once per window size."""
    ...
    def render_sun_glare(self):
        """The Sun is a marker, not a scene element. Drawn at a small fixed
        pixel size so it stays legible and never swallows the view, no matter
        how far the operator has zoomed in."""
        sra, sdec = self.sun_pos()
        x, y = self.world_to_screen(sra, sdec)
        if not (-40 < x < self.L.w + 40 and self.L.sky.top - 40 < y < self.L.sky.bottom + 40):
            return
        # fixed pixel footprint, independent of zoom
    ...
    def draw_star(self, o, x, y, st):
        ...
        b = st * (1 - dim)
        r = clamp((7.6 - o.mag) * 0.52, 0.6, 5.4) * clamp(0.75 + math.log10(max(self.zoom, 1.1)) * 0.45, 0.6, 1.7)
        col = dim_color(lerp_color(o.color, (10, 10, 12), dim * 0.8), clamp(b, 0.08, 1))
        ...
        if o.variable and o.stage >= 2:
            ring = RED_WARN if (o.is_dimmed(self.game_hours) and o.stage < 3) else AMBER_DIM
            pygame.draw.circle(self.screen, ring, (int(x), int(y)), int(r + 7 * self.L.scale), 1)
        show_label = (o.stage >= 2) or (not o.proc and self.zoom > 7) or self.zoom > 60
        if show_label and b > 0.3:
            mark = ""
            if o.stage >= 2:
                mark = "  *" if not o.variable else ("  [SOLVED]" if o.stage >= 3 else "  [VARIABLE]")
        ...
        if k == "galaxy":
            ...
            for a in (0, math.pi / 2, math.pi, 3 * math.pi / 2):
                pygame.draw.line(self.screen, col, (x, y),
                                 (x + math.cos(a) * s * 1.6, y + math.sin(a) * s * 1.6), 1)
        elif k == "nebula":
            ...
            for i in range(6):
                ...
                pygame.draw.circle(self.screen, col,
                                   (int(x + math.cos(a) * s * 0.5), int(y + math.sin(a) * s * 0.5)), 1)
        elif k == "open":
            for i in range(5):
                a = i * 1.257
                pygame.draw.circle(self.screen, col,
                                   (int(x + math.cos(a) * s), int(y + math.sin(a) * s)), 1)
        elif k == "remnant":
            pygame.draw.circle(self.screen, col, (int(x), int(y)), int(s), 1)
            pygame.draw.circle(self.screen, dim_color(col, 0.5), (int(x), int(y)), int(s * 0.55), 1)
        elif k == "exotic":
    ...
    def draw_anomaly(self, o, x, y, st, mouse):
        ...
        draw_reticle(self.screen, (x, y), size + 5 * self.L.scale, col, rot=t * 0.6, gap=0.5, width=1)
    ...
    # ---- reticle, spectrometer, signal meter ------------------------------
    def render_reticle(self):
        ...
        if self.L.panel_open and self.L.panel.collidepoint(mouse):
            return
        if self.monitor_hit(mouse):
            return
        if self.debug_hit(mouse):
            return
        if self.monitor_mode:
        ...
        pygame.draw.line(self.screen, col, (mouse[0] - 4, mouse[1]), (mouse[0] + 4, mouse[1]), 1)
        pygame.draw.line(self.screen, col, (mouse[0], mouse[1] - 4), (mouse[0], mouse[1] + 4), 1)
        ...
        if sig > 0.02:
            ...
            pygame.draw.rect(self.screen, c, (bx + 1, by + 1, int((bw - 2) * sig), bh - 2))

        if self.interference:
            text_at(self.screen, self.fonts["sm"], "NO CARRIER", (mouse[0] + 24, mouse[1] + 16), RED_WARN)
            return
    ...
    def render_reticle_monitor(self, mouse):
        """Alternate crosshair drawn while monitor-scan mode (R) is active."""
        o = self.mon_target
        ...
        draw_reticle(self.screen, mouse, R, col, rot=-self.session_t * 0.6, gap=0.6, width=1)
        text_at(self.screen, self.fonts["xs"], "M",
                (mouse[0], mouse[1] - R - int(15 * self.L.scale)), col, align="center")
        ...
        if o is not None:
            if self.mon_scan_p > 0 and not already:
                ...
                pygame.draw.arc(self.screen, CYAN, rect, -math.pi / 2,
                                -math.pi / 2 + frac * 2 * math.pi, 3)
            label = o.name if (o.kind != "anomaly" or o.stage >= 1) else "UNIDENTIFIED CONTACT"
            msg = "%s -- already monitoring" % label if already else "%s -- hold to MONITOR" % label
            text_at(self.screen, self.fonts["sm"], msg, (mouse[0] + 24, mouse[1] + 16),
    ...
    def describe(self, o):
        ...
        if o.kind == "anomaly":
            ...
            return "UNKNOWN CONTACT -- hold to scan%s" % band
        if o.kind == "star" and o.variable and o.stage >= 2 and o.is_dimmed(self.game_hours):
            return "%s -- LUMINOSITY DROP IN PROGRESS -- hold to investigate" % o.name
    ...
    def render_spectrometer(self, o):
        """Live readout while a scan is charging: waveform + redshifted lines."""
        ...
        head = "SPECTROMETER  //  %s  //  %s" % (self.mode, o.name if o.kind != "anomaly" else "CONTACT")
        ...
        for i in range(0, gw, 2):
            ...
            # emission bumps at the redshifted line positions
            if match:
        ...
        if len(pts) > 1:
            pygame.draw.lines(self.screen, MODE_COLORS[self.mode] if match else PHOS_FAINT,
                              False, pts, 1)
        if match:
            ...
            text_at(self.screen, self.fonts["xs"],
                    "H-alpha 656nm -> %.0f nm  [%s]   z=%.4f" % (ha, band_label(ha), z),
    ...
    def render_monitors(self):
        """Small, non-intrusive dropdown-style cards stacked top-left of the
        sky view: one live activity graph + band rosette per linked contact."""
        if not self.monitors:
            return
        L = self.L
        ...
        for m in self.monitors:
            ...
            if len(label) > 20:
                label = label[:19] + "."
            text_at(self.screen, self.fonts["xs"], label, (rect.x + 6, rect.y + 4), CYAN, glow=False)
            ...
            hist = m["hist"]
            if len(hist) > 1:
                pts = [(gx + (i / max(1, MONITOR_HIST_LEN - 1)) * gw, gy + gh - v * gh)
                ...
                text_at(self.screen, self.fonts["xs"], "acquiring...", (gx, gy + gh + 2),
                        PHOS_DIM, glow=False)

            # band rosette -- which sensor this contact actually answers on
    ...
    def render_hud(self):
        ...
        ra, dec = self.screen_to_world(*pygame.mouse.get_pos())
        h = ra / 15.0
        pos = "RA %02dh%02dm   DEC %+05.1f   FOV %.1f deg   zoom %.0f%%" % (
            int(h), int((h - int(h)) * 60), dec, (L.w / self.zoom), self.zoom * 10)
        text_at(self.screen, self.fonts["xs"], pos, (L.w * 0.34, int(14 * L.scale)), PHOS_DIM, glow=False)
        ...
        if self.settings["hints"] and not self.interference:
            text_at(self.screen, self.fonts["xs"], HINTS[self.hint_i],
                    (L.w // 2, L.sky.bottom - 18), PHOS_DIM, align="center", glow=False)
    ...
    def render_bottom(self):
        ...
        self.mode_rects = []
        bx = 14
        by = r.top + int(10 * L.scale)
        for i, m in enumerate(MODES):
            bw, bh = int(104 * L.scale), int(26 * L.scale)
            rect = pygame.Rect(bx, by, bw, bh)
            active = (m == self.mode)
            col = MODE_COLORS[m]
            panel_rect(self.screen, rect, border=col if active else PHOS_FAINT,
                       fill=(col[0] // 8, col[1] // 8, col[2] // 8, 200 if active else 90))
            text_at(self.screen, self.fonts["xs"], "%d %s" % (i + 1, m),
                    (rect.centerx, rect.y + int(6 * L.scale)),
                    col if active else PHOS_DIM, align="center", glow=active)
            self.mode_rects.append((rect, m))
            bx += bw + 6
        text_at(self.screen, self.fonts["xs"], "SENSOR BANK", (14, by + int(30 * L.scale)),
                PHOS_DIM, glow=False)

        # --- redshift dial: a rotary "Z" gauge, not a slider ---------------
        R = int(clamp(L.bottom_h * 0.34, 20, 32))
        dcx = bx + int(20 * L.scale) + R
        dcy = r.top + int(L.bottom_h * 0.42)
        ...
        pygame.draw.lines(self.screen, PHOS_FAINT, False, [_dial_pt(i / 40.0) for i in range(41)], 1)
        ...
        for zt, lab in ((0.0, "0"), (0.02, ".02"), (0.16, ".16"), (1.0, "1"),
            ...
            p = self.z_to_p(zt)
            pygame.draw.line(self.screen, PHOS_DIM, _dial_pt(p, 0.80), _dial_pt(p, 1.0), 1)
        ...
        text_at(self.screen, self.fonts["xs"], "0  .02  .16  1  7  100  CMB",
                (dcx, dcy + R + int(4 * L.scale)), PHOS_FAINT, align="center", glow=False)
        text_at(self.screen, self.fonts["xs"], "Q/E, drag, or scroll", (dcx, dcy - R - int(16 * L.scale)),
                PHOS_DIM, align="center", glow=False)

        # --- cosmology instrument cluster (other "dials" derived from Z) ---
        z = self.z_dial
        rx = dcx + R + int(22 * L.scale)
        if rx < L.w - 130:
            lb = lookback_gyr(z)
            ...
            lines = [
                ...
                "v_rec %.0f km/s (%.3fc) %s" % (v, vfrac, bar_str(vfrac, 8)),
                "scale a = %.4f %s" % (afrac, bar_str(afrac, 8)),
            ]
            for i, s in enumerate(lines):
                text_at(self.screen, self.fonts["xs"], s, (rx, r.top + 8 + i * int(14 * L.scale)),
    ...
    def render_panel(self):
        ...
        for i, name in enumerate(tabs):
            ...
            text_at(self.screen, self.fonts["xs"], name, (tr.centerx, tr.y + int(5 * L.scale)),
                    col, align="center", glow=active)
            self.tab_rects.append((tr, i))
        pygame.draw.line(self.screen, PHOS_FAINT, (rect.x, rect.y + int(24 * L.scale)),
    ...
    def render_tab_log(self, b):
        ...
        for e, lines in shown:
            text_at(self.screen, self.fonts["xs"], e.stamp, (b.x, y), PHOS_FAINT, glow=False)
            top = y
            for i, ln in enumerate(lines):
                text_at(self.screen, self.fonts["xs"], ln, (b.x + 54, y + i * line_h), e.color, glow=False)
    ...
    def render_tab_data(self, b):
        ...
        for k, v in rows:
            ...
            text_at(self.screen, self.fonts["xs"], v, (b.right, y), PHOS, align="right", glow=False)
            y += int(14 * L.scale)
        y += int(8 * L.scale)
        text_at(self.screen, self.fonts["xs"], "RECENT ENTRIES  (click to recentre)", (b.x, y), AMBER_DIM, glow=False)
        y += int(16 * L.scale)
        ...
        for o in reversed(recent):
            ...
            r = text_at(self.screen, self.fonts["xs"], label, (b.x, y), col, glow=False)
    ...
    def render_tab_calib(self, b):
        ...
        if not self.interference and not self.calib:
            text_at(self.screen, self.fonts["sm"], "ALL CHANNELS NOMINAL", (b.x, b.y), PHOS)
            ...
            for k, v in rows:
                ...
                y += int(14 * L.scale)
            y += int(10 * L.scale)
            for ln in wrap_text("If broadband interference hits, this panel is where you "
                                "re-phase the three receiver channels. Nothing else works "
                                "until the carrier is back.", self.fonts["xs"], b.w):
                text_at(self.screen, self.fonts["xs"], ln, (b.x, y), PHOS_FAINT, glow=False)
                y += int(13 * L.scale)
            self.calib_rects = []
            return
        ...
        text_at(self.screen, self.fonts["xs"], "W/S pick channel   A/D slide   SPACE lock",
        ...
        for i, band in enumerate(c["bands"]):
            ...
            # target waveform (ghost) and current waveform
    ...
    def render_tab_help(self, b):
        ...
        lines = [
            ...
            ("REDSHIFT", "Q / E   (SHIFT coarse, Z reset, scroll dial to trim)"),
            ("MONITOR", "R toggles monitor-scan; hold LMB to link a graph"),
            ("PANEL", "TAB cycles   H hides"),
            ("FILTER", "F cycles the log filter"),
            ("SAVE", "F5 quick save   F9 quick load"),
            ("MENU", "ESC or M"),
            ("REPORTBUGS", "tjbinkadink@gmail.com") 
        ]
        y = b.y
        for k, v in lines:
            text_at(self.screen, self.fonts["xs"], k, (b.x, y), AMBER_DIM, glow=False)
            text_at(self.screen, self.fonts["xs"], v, (b.x + int(72 * L.scale), y), PHOS_DIM, glow=False)
            y += int(14 * L.scale)
        y += int(10 * L.scale)
        txt = ("Contacts answer on one band. A red giant is loudest on THERMAL, a "
               "quasar on RADIO, most stars on VISUAL. Wrong band, slow scan.\n"
               "Anomalies take two passes: classify, wait about three in-game hours, "
               "then resolve.\n"
               "The dial is depth. z=0 is the local sky; turn it up and you trade "
               "the nearby stars for the deep universe. At z=1089 there is nothing "
               "further to see.")
    ...
    def render_crt(self):
        ...
        if self.state == "PLAY" and self.interference:
            # Restrict the heavy interference static to the sky view. The side
            # panel and bottom bar must stay legible so the CALIB tab can be
            # read and clicked while the carrier is down.
        ...
        if getattr(self, "glitch_flash", 0) > 0:
            ...
            s.fill((PHOS[0], PHOS[1], PHOS[2], int(28 * clamp(self.glitch_flash / 0.16, 0, 1))))
    ...
    def button(self, rect, label, sub="", active=False, danger=False):
        ...
        if sub:
            text_at(self.screen, self.fonts["xs"], sub,
                    (rect.right - int(14 * self.L.scale), rect.y + int(13 * self.L.scale)),
                    PHOS_DIM, align="right", glow=False)
        return rect
    ...
    def render_menu(self):
        ...
        cx = int(L.w * 0.5)
        text_at(self.screen, self.fonts["title"], "VYSTLZ", (cx, int(L.h * 0.10)), PHOS, align="center")
        text_at(self.screen, self.fonts["title"], "DEEP SKY", (cx, int(L.h * 0.10 + 52 * L.scale)),
                AMBER, align="center")
        text_at(self.screen, self.fonts["xs"],
                "ground array  //  observation only  //  v%s" % VERSION,
                (cx, int(L.h * 0.10 + 112 * L.scale)), PHOS_DIM, align="center", glow=False)
        ...
        if self.menu_page == "MAIN":
            ...
            items = [("new", "NEW SURVEY", "fresh seed"),
                     ("continue", "CONTINUE", "most recent save" if has_save else "no saves"),
                     ("load", "LOAD SURVEY", ""),
                     ("settings", "SETTINGS", ""),
                     ("about", "ABOUT", ""),
                     ("quit", "QUIT", "esc")]
            ...
            text_at(self.screen, self.fonts["md"], "SELECT SLOT", (cx, y - int(34 * L.scale)),
                    PHOS, align="center")
            ...
            for key, lab in (("crt", "CRT FILTER"), ("audio", "AUDIO"),
                             ("hints", "HINT LINE")):
                r = pygame.Rect(x, y, bw, bh)
                self.buttons.append((self.button(r, lab, "ON" if self.settings[key] else "OFF",
                                                 active=self.settings[key]), "set_" + key))
                y += gap
            self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), "FULLSCREEN (F11)",
                                             "on" if self.fullscreen else "off"), "fullscreen"))
            y += gap
            self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), "BACK"), "back"))
        elif self.menu_page == "ABOUT":
            txt = ("You run a ground receiver array. There is no ship, no fuel, no oxygen "
                   "and nothing out here can reach you. You point the dish, pick a band, "
                   "tune the redshift, and write down what answers.\n\n"
                   "The catalogue is real: star positions, deep sky objects, redshifts and "
                   "distances are the published figures, and the cosmology readouts are "
                   "integrated from a flat LCDM model. The anomalies are not real. "
                   "Probably.\n\n"
                   "Made for someone with no cosmology degree and no patience for clutter.")
            yy = int(L.h * 0.34)
            for para in txt.split("\n"):
                for ln in wrap_text(para, self.fonts["sm"], int(L.w * 0.6)):
                    text_at(self.screen, self.fonts["sm"], ln, (cx, yy), PHOS_DIM,
                            align="center", glow=False)
                    yy += int(19 * L.scale)
            self.buttons.append((self.button(pygame.Rect(x, int(L.h * 0.80), bw, bh), "BACK"), "back"))

        if self.menu_msg_t > 0:
            text_at(self.screen, self.fonts["sm"], self.menu_msg, (cx, L.h - int(40 * L.scale)),
                    RED_WARN, align="center")
    ...
    def render_pause(self):
        ...
        text_at(self.screen, self.fonts["xl"], "ARRAY PAUSED", (cx, int(L.h * 0.16)), PHOS, align="center")
        text_at(self.screen, self.fonts["xs"],
                "%s    score %06d    seed %d" % (self.clock_str(), self.score, self.seed),
                (cx, int(L.h * 0.16 + 44 * L.scale)), PHOS_DIM, align="center", glow=False)
        ...
        if page == "MAIN":
            items = [("resume", "RESUME", "esc"), ("save", "SAVE SURVEY", "F5 quick"),
                     ...
                     ("menu", "MAIN MENU", "unsaved progress lost"), ("quit", "QUIT", "")]
            for key, lab, sub in items:
                self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), lab, sub,
                                                 danger=(key in ("menu", "quit"))), "p_" + key))
                y += gap
        elif page in ("SAVE", "LOAD"):
            text_at(self.screen, self.fonts["md"], page + " SLOT", (cx, y - int(30 * L.scale)),
                    AMBER, align="center")
            for i in range(1, SAVE_SLOTS + 1):
                m = self.save_meta(i)
                sub = "empty" if not m else "%s  D%02d  %d pts" % (
                    m["saved"], int(m["hours"] // 24) + 1, m["score"])
                self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), "SLOT %d" % i, sub),
                                     "%s%d" % (page.lower(), i)))
                y += gap
            ...
            for key, lab in (("crt", "CRT FILTER"), ("audio", "AUDIO"),
                ...
                self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), lab,
                                                 ...
                                                 active=self.settings[key]), "pset_" + key))
                y += gap
            self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), "FULLSCREEN (F11)",
                                             "on" if self.fullscreen else "off"), "p_fullscreen"))
            y += gap
            self.buttons.append((self.button(pygame.Rect(x, y, bw, bh), "BACK"), "p_back"))

        if self.toast_t > 0:
            text_at(self.screen, self.fonts["sm"], self.toast, (cx, L.h - int(46 * L.scale)),
                    AMBER, align="center")
